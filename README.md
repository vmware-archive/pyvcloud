...
Usage examples:
    ...
    |   38 | add network                          | vca network add --network network_name --gateway gateway_name --gateway_ip 192.168.117.1 --netmask 255.255.255.0 --dns1 192.168.117.1 --pool 192.168.117.2-192.168.117.100 |
    ...
    |   45 | add edge gateway DNAT rule           | vca nat add --type DNAT --original_ip 107.189.93.162 --original_port 22 --translated_ip 192.168.109.2 --translated_port 22 --protocol tcp                                  |
    |   46 | add edge gateway SNAT rule           | vca nat add --type SNAT --original_ip 192.168.109.0/24 --translated_ip 107.189.93.162                                                                                      |
    |   47 | add edge gateway rules from file     | vca nat add --file natrules.yaml                                                                                                                                           |
    |   48 | delete edge gateway NAT rule         | vca nat delete --type DNAT --original_ip 107.189.93.162 --original_port 22 --translated_ip 192.168.109.4 --translated_port 22 --protocol tcp                               |
    ...
    |   57 | add VPN tunnel                       | vca vpn add-tunnel --tunnel t1 --local_ip 107.189.123.101 --local_network routed-116 --peer_ip 192.240.158.15 --peer_network 192.168.110/24 --secret P8s3P...7v            |
    |   58 | delete VPN tunnel                    | vca vpn del-tunnel --tunnel t1                                                                                                                                             |
    |   59 | add local network to VPN tunnel      | vca vpn add-network --tunnel t1 --local_network routed-115                                                                                                                 |
    |   60 | add peer network to VPN tunnel       | vca vpn add-network --tunnel t1 --peer_network 192.168.115.0/24                                                                                                            |
    |   61 | delete local network from VPN tunnel | vca vpn del-network --tunnel t1 --local_network routed-115                                                                                                                 |
    |   62 | delete peer network from VPN tunnel  | vca vpn del-network --tunnel t1 --peer_network 192.168.115.0/24                                                                                                            |
