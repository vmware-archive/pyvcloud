...
class VApp(object):
    ...
    def enable_dhcp_vapp_network(self, network_name, ip_range,
        ...
        for network_config in self.resource.NetworkConfigSection.NetworkConfig:
            if network_config.get('networkName') == network_name:
                ...
                if not hasattr(network_config.Configuration, 'Features'):
                    index = network_config.Configuration.index(
                        network_config.Configuration.GuestVlanAllowed)
                    network_config.Configuration.insert(index, E.Features())
                if not hasattr(network_config.Configuration.Features,
                               'DhcpService'):
                    network_config.Configuration.Features.append(
                        E.DhcpService())
                dhcp = network_config.Configuration.Features.DhcpService
                if hasattr(dhcp, 'IsEnabled'):
                    dhcp.IsEnabled = E.IsEnabled(True)
                else:
                    dhcp.append(E.IsEnabled(True))
                if hasattr(dhcp, 'DefaultLeaseTime'):
                    dhcp.DefaultLeaseTime = E.DefaultLeaseTime(
                        default_lease_time)
                else:
    ...
    def disable_dhcp_vapp_network(self, network_name):
        """Enable DHCP to vApp network.
