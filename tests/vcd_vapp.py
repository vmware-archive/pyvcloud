...
class TestVApp(TestCase):
    ...
    def test_013_add_disk(self):
        ...
        vapp = VApp(self.client, resource=vapp_resource)
        disk_size = 1024  # 1GB
        result = vapp.add_disk_to_vm(
            vm_name, disk_size, disk_controller="lsilogicsas")
        task = self.client.get_task_monitor().wait_for_status(
            task=result,
            timeout=60,
            poll_frequency=2,
    ...
    def test_014_add_disk(self):
        ...
        disk_size = 1024  # 1GB
        result = vapp.add_disk_to_vm(
            vm_name, disk_size, disk_controller="VirtualSCSI")
        task = self.client.get_task_monitor().wait_for_status(
            task=result,
            timeout=60,
            poll_frequency=2,
