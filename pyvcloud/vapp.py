...
class VAPP(object):
    ...
    def execute(self, operation, http, body=None, targetVM=None):
        ...
        if not link:
            self.logger.error("vapp.execute: link not found; rel=%s" % operation)
            self.logger.debug("vapp.execute: vApp href=%s, name=%s" % vApp.get_href(), vApp.get_name())
            return False
        else:
            if http == "post":
            ...
            if self.response.status_code == requests.codes.accepted:
                ...
                self.logger.debug("vapp.execute: failed; response status=%d, content=%s" % (self.response.status_code, response.text))
                return False
    ...
    def disconnect_vms(self, network_name):
        ...
        if children:
            vms = children.get_Vm()
            for vm in vms:
                self.logger.debug("vapp.disconnect_vms: child VM name=%s" % vm.get_Name())
                # new_connection = self._create_networkConnection(
                #     network_name, connection_index, ip_allocation_mode,
                #     mac_address, ip_address)
                # networkConnectionSection = [section for section in vm.get_Section() if isinstance(section, NetworkConnectionSectionType)][0]
        ...
        if children:
            ...
            if len(vms) == 1:
                ...
                if self.response.status_code == requests.codes.accepted:
                    ...
                    self.logger.debug("vapp.customize_guest_os: failed; response status=%d, content=%s" % (self.response.status_code, self.response.text))
            ...
            if len(vms) == 1:
                ...
                if len(links) == 1:
                    ...
                    if self.response.status_code == requests.codes.accepted:
                        ...
                        self.logger.debug("vapp.force_customization: failed; response status=%d, content=%s" % (self.response.status_code, self.response.text))
    ...
    def customize_on_next_poweron(self):
        ...
        self.logger.error("vapp.customize_on_next_poweron: link not found")
        return False
