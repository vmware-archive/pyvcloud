...
Ext = objectify.ElementMaker(annotate=False,
...
class Extension(object):
    ...
    def list_extensions(self):
        query = self.client.get_typed_query(self.TYPE_NAME,
            query_result_format=QueryResultFormat.ID_RECORDS)
        return [ to_dict(r, self.ATTRIBUTES) for r in query.execute() ]

    def get_extension(self, name):
        ext = self.client.get_typed_query(self.TYPE_NAME,
    ...
    def add_extension(self, name, namespace, routing_key, exchange, patterns):
        ...
        ext_services = self.client.get_linked_resource(ext,
                                RelationType.DOWN,
                                EntityType.EXTENSION_SERVICES.value)
        return self.client.post_linked_resource(ext_services,
                                RelationType.ADD,
